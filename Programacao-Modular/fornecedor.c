...
void pedeFornecedor(char* nomeJogo){
    ...
    cJSON* item = cJSON_GetArrayItem(listaJson, 0);
    if (item == NULL) {
        printf("Erro ao obter o item do array.\n");
        cJSON_Delete(listaJson);
        exit(-1);
    }

    cJSON_ReplaceItemInObject(item, "nome", cJSON_CreateString(nomeJogo));
    cJSON_ReplaceItemInObject(item, "quantidade", cJSON_CreateNumber(1));
    cJSON_ReplaceItemInObject(item, "preco", cJSON_CreateNumber(-1));
    cJSON_ReplaceItemInObject(item, "finalizacao", cJSON_CreateNumber(0));
...
void recebeRequisicao(Estoque* lista){
    ...
    cJSON* finalizacao = cJSON_GetObjectItem(item, "finalizacao");
    if (finalizacao == NULL) {
    ...
    if (finalizacao->valueint == 1){
        ...
        cJSON* preco = cJSON_GetObjectItem(item, "preco");
        char nomeAux[50];
        ...
        while (aux != NULL) {
            if(strcmp(nomeAux, aux->dados.nome) == 0){
                ...
                arquivo = abreArq(CAMINHO, "w");
                cJSON_ReplaceItemInObject(item, "nome", cJSON_CreateString(""));
                cJSON_ReplaceItemInObject(item, "quantidade", cJSON_CreateNumber(-1));
                cJSON_ReplaceItemInObject(item, "preco", cJSON_CreateNumber(-1));
                cJSON_ReplaceItemInObject(item, "finalizacao", cJSON_CreateNumber(-1));
                char* novoJson = cJSON_Print(listaJson);
                arquivo = abreArq(CAMINHO, "w");
                fprintf(arquivo, "%s", novoJson);
                fclose(arquivo);
